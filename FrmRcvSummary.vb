Public Class FrmRcvSummary
    ...
    Private Sub FrmOpenra_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Me.ComboBoxEx2.Items.Add("일반")
        Me.ComboBoxEx2.Items.Add("폴리싱")
        Me.ComboBoxEx2.Items.Add("RMA")
        Me.ComboBoxEx2.Items.Add("구매품")
    ...
    Private Sub FindBtn_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles FindBtn.Click
        ...
        qry = qry & "	and SUBSTRING(PSHIP_NO,4,8) between  convert(varchar(10), convert(datetime, '" & DateTimeInput1.Text & "'), 112) and convert(varchar(10), convert(datetime, '" & DateTimeInput2.Text & "'), 112)" & vbNewLine
        If m_qry <> "" Then
            qry = qry & "AND MODEL IN (" & m_qry & ")" & vbNewLine
        End If
        If ComboBoxEx2.Text <> "ALL" Then
            If ComboBoxEx2.Text = "일반" Then
                qry = qry & "AND OUT_ESN NOT LIKE 'S%'" & vbNewLine
            ElseIf ComboBoxEx2.Text = "폴리싱" Then
                qry = qry & "AND OUT_ESN LIKE 'S%'" & vbNewLine
            ElseIf ComboBoxEx2.Text = "구매품" Then
                qry = qry & "AND OUT_ESN LIKE 'C%'" & vbNewLine
            ElseIf ComboBoxEx2.Text = "RMA" Then
                qry = qry & "AND QAREJECT = 'Y'" & vbNewLine
            End If
        ...
        If Query_Spread(FpSpread1, qry, 1) = True Then
            'If Query_Spread(FpSpread1, "exec KSP_FRMRCVSUMMARY_GETSUMMARY '" & Site_id & "', '" & DateTimeInput1.Text & "', '" & DateTimeInput2.Text & "','" & ComboBoxEx1.Text & "'", 1) = True Then
    ...
    Private Sub FpSpread1_CellDoubleClick(ByVal sender As Object, ByVal e As FarPoint.Win.Spread.CellClickEventArgs) Handles FpSpread1.CellDoubleClick
        ...
        If Query_Spread(FpSpread3, "exec KSP_FRMRCVSUMMARY_GETDAILY '" & Site_id & "', '" & FpSpread1.ActiveSheet.GetValue(e.Row, 0) & "', '" & FpSpread1.ActiveSheet.GetValue(e.Row, 0) & "','" & FpSpread1.ActiveSheet.GetValue(e.Row, 1) & "','" & FpSpread1.ActiveSheet.GetValue(e.Row, 2) & "'", 1) = True Then
    ...
    Private Sub FpSpread3_CellDoubleClick(ByVal sender As Object, ByVal e As FarPoint.Win.Spread.CellClickEventArgs) Handles FpSpread3.CellDoubleClick
        ...
        qry = qry & "Select OUT_esn, model,qareject, PSHIP_NO, OUTBOX_NO, CONVERT(INT,OUT_POS), OUT_DEF_CD, CHK_RCV, ISNULL(RCV_DATE,''), ISNULL(PBOX_NO,''), ISNULL(CSHIP_NO,''),  ISNULL(IN_ESN,'') " & vbNewLine
        qry = qry & "from VIEW_Fesnmaster" & vbNewLine
        qry = qry & "where site_id = '" & Site_id & "'" & vbNewLine
