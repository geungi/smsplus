Public Class FrmQCLine
    ...
    Private Sub FrmQCLine_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        'End If
        If Query_Combo(Me.ComboBoxEx5, "SELECT CODE_NAME FROM TBL_CODEMASTER WHERE site_id = '" & Site_id & "' and CLASS_ID = 'R0001' AND active = 'Y' AND CODE_ID IN ('K8000','K8300') ORDER BY CODE_ID") = True Then
            Me.ComboBoxEx5.SelectedIndex = 0
        End If
        ...
        If LOT_VERIFY(Me.TextBoxX3, e) = True Then
            ...
            With FpSpread2.ActiveSheet
                For I As Integer = 0 To FpSpread4.ActiveSheet.RowCount - 1
                    If Insert_Data("EXEC SP_COMMON_LOTSAVE '" & Site_id & "','" & TextBoxX3.Text & "','" & FpSpread4.ActiveSheet.GetValue(I, 0) & "','','N/A',0,'1차 검사 불량','" & Emp_No & "','LCD FUNCTION TEST(1)','" & ComboBoxEx5.Text & "','" & OP_No & "','C1000', " & FpSpread4.ActiveSheet.GetValue(I, FpSpread4.ActiveSheet.ColumnCount - 1)) = True Then
                    End If

                    If Insert_Data("exec SP_COMMON_WKRESULT_FAIL '" & Site_id & "','LCD FUNCTION TEST(1)','" & OP_No & "','" & qc_rs(0).Value & "','" & TextBoxX3.Text & "'," & FpSpread4.ActiveSheet.GetValue(I, FpSpread4.ActiveSheet.ColumnCount - 1)) = True Then
                    End If
                ...
                'If Insert_Data("EXEC SP_COMMON_LOTSAVE '" & Site_id & "','" & TextBoxX3.Text & "','" & FpSpread4.ActiveSheet.GetValue(0, 0) & "','','N/A',0,'1차 검사 불량','" & Emp_No & "','LCD FUNCTION TEST(1)','" & ComboBoxEx5.Text & "','" & OP_No & "','C1000', " & IntegerInput1.Text) = True Then
                'End If

                'If Insert_Data("exec SP_COMMON_WKRESULT_FAIL '" & Site_id & "','LCD FUNCTION TEST(1)','" & OP_No & "','" & qc_rs(0).Value & "','" & TextBoxX3.Text & "'," & IntegerInput1.Text) = True Then
                'End If
