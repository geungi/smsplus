...
'                    S3.Cells(S3.RowCount - 1, 2).Text = S2.Cells(rowidx, 5).Text
                    ...
                    Chg_ComboCell(FpSpread3, S3.RowCount - 1, 3, Query_Cell_Code2("select '['+code_id+'] ' +code_name AS CD from tbl_codemaster where class_id = '10024' and code_id <> '" & Mid(PtWhCb.Text, 2, 5) & "' UNION SELECT '[U2014-0001] 엑소더스 와이어레스' AS CD ORDER BY CD "))
            ...
            If S3.RowCount > 0 Then
                ...
                For i = 0 To S3.RowCount - 1
                    ...
                    If S3.Rows(i).ForeColor = Color.OrangeRed Then
                        ...
                        If Insert_Data("EXEC SP_FRMPARTTRANSFER_INSINV '" & Site_id & "','" & S3.Cells(i, 0).Text & "','Trans-" & Mid(S3.Cells(i, 2).Text, 2, 5) & Mid(S3.Cells(i, 3).Text, 2, 5) & "'," & S3.Cells(i, 1).Value & ", '" & Mid(S3.Cells(i, 2).Text, 2, 5) & "','" & Mid(S3.Cells(i, 3).Text, 2, 5) & "','" & S3.Cells(i, 4).Text & "','" & Emp_No & "'") = False Then
