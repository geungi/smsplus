Public Class FrmBCConf
    ...
    Private Sub FindBtn_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles FindBtn.Click
        Try
            ...
            qry = "SELECT SITE_id, model, bc_type, cn_type, " & vbNewLine
            ...
            qry = qry & "FROM TBL_BCCONFIG A " & vbNewLine
            ...
            qry = qry & "ORDER BY model, bc_type, cn_type" & vbNewLine
        ...
        Try
            ...
            Select Case e.Column
                Case 2
                    ...
                    Chg_ComboCell(FpSpread1, e.Row, 2, New String() {"생산LOT", "출고플립ID"})
                Case 3
                    Me.FpSpread1.ActiveSheet.Cells(e.Row, e.Column).Locked = False
    ...
    Private Sub FpSpread1_Change(ByVal sender As Object, ByVal e As FarPoint.Win.Spread.ChangeEventArgs) Handles FpSpread1.Change
        Try
            ...
            Me.FpSpread1.ActiveSheet.Cells(e.Row, 6).Text = Emp_No
            Me.FpSpread1.ActiveSheet.Cells(e.Row, 7).Text = Now
            ...
            For i = 0 To rowCnt - 1
                ...
                With FpSpread1.ActiveSheet
                    If .Rows(i).ForeColor = Color.OrangeRed Then
                        ...
                        QRY = QRY & "IF EXISTS (SELECT MODEL FROM TBL_BCCONFIG WHERE MODEL = '" & .GetValue(i, 1) & "' AND BC_TYPE = '" & .GetValue(i, 2) & "')" & vbNewLine
                        QRY = QRY & "BEGIN" & vbNewLine
                        QRY = QRY & "   UPDATE TBL_BCCONFIG SET CN_TYPE = '" & .GetValue(i, 3) & "' WHERE MODEL = '" & .GetValue(i, 1) & "' AND BC_TYPE = '" & .GetValue(i, 2) & "'" & vbNewLine
                        QRY = QRY & "END" & vbNewLine
                        QRY = QRY & "ELSE" & vbNewLine
                        QRY = QRY & "BEGIN" & vbNewLine
                        QRY = QRY & "   INSERT INTO TBL_BCCONFIG VALUES ('" & .GetValue(i, 0) & "','" & .GetValue(i, 1) & "','" & .GetValue(i, 2) & "','" & .GetValue(i, 3) & "',NULL,NULL,'" & Emp_No & "', GETDATE(), '" & Emp_No & "', getdate())" & vbNewLine
                        QRY = QRY & "END" & vbNewLine
    ...
    Private Sub NewBtn_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles NewBtn1.Click, NewBtn.Click, bNew.Click
        Try
            ...
            With Me.FpSpread1.ActiveSheet
                ...
                .Cells(.RowCount - 1, 1).Value = ClassCb.Text

                Chg_ComboCell(FpSpread1, .RowCount - 1, 2, New String() {"생산LOT", "출고플립ID"})

                .Cells(.RowCount - 1, 4).Text = Emp_No
                .Cells(.RowCount - 1, 5).Text = Now
            ...
            With FpSpread1.ActiveSheet
                If (site_id <> "") And (class_id <> "") Then
                    ...
                    If answer = Windows.Forms.DialogResult.Yes Then
                        Insert_Data("delete from TBL_BCCONFIG WHERE MODEL = '" & .GetValue(.ActiveRowIndex, 1) & "' AND BC_TYPE = '" & .GetValue(.ActiveRowIndex, 2) & "'")
